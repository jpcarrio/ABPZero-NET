...
namespace ISUCore.EntityFrameworkCore.Seed.Tenants
{
    public class TenantRoleAndUserBuilder
    {
        ...
        public TenantRoleAndUserBuilder(ISUCoreDbContext context, int tenantId)
        {
            _context = context;
            _tenantId = tenantId;
        }
        ...
        private void CreateRolesAndUsers()
        {
            ...
            var permissions = PermissionFinder
                .GetAllPermissions(new ISUCoreAuthorizationProvider())
            ...
            if (adminUser == null)
            {
                var email = $"{AbpUserBase.AdminUserName}{_tenantId}@{ISUCoreConsts.mailService}.com";
                adminUser = User.CreateTenantAdminUser(_tenantId, email);
                adminUser.Password = new PasswordHasher<User>(new OptionsWrapper<PasswordHasherOptions>(new PasswordHasherOptions())).HashPassword(adminUser, "isucore1!");
            ...
            // ApiClient user
            //
            var apiClientUser = _context.Users.IgnoreQueryFilters().FirstOrDefault(u => u.TenantId == _tenantId && u.UserName == ISUCoreConsts.DefaultApiUserName);
            if (apiClientUser == null)
            {
                var email = $"{ISUCoreConsts.DefaultApiUserName}{_tenantId}@{ISUCoreConsts.mailService}.com";
                apiClientUser = User.CreateTenantUser(_tenantId, email, ISUCoreConsts.DefaultApiUserName);
                apiClientUser.Password = new PasswordHasher<User>(new OptionsWrapper<PasswordHasherOptions>(new PasswordHasherOptions())).HashPassword(apiClientUser, ISUCoreConsts.DefaultApiPassword);
